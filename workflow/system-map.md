...
규칙이 반복해서 안 지켜지면 한 단계 아래로 내린다. 이번 개편에서 파괴적 git 명령 가드가 문장(전역 CLAUDE.md 체크리스트)에서 훅으로 내려간 것이 그 예이고, 2026-08-30에 지시서 크기 규칙이 같은 경로로 내려갔다(아래 §7). 사용자에게 물어야 하는 규칙도 훅으로 내릴 수 있는데, 이때 훅은 명령을 막는 대신 정한 시점에 세션에 알림을 넣는다(아래 §8).
...
7. **`/track`과 `/slack-feedback`**: `/track`은 확인 묶음의 항목 가운데 db, 로그, 슬랙 조회로 판정할 수 있는 것은 직접 닫고, 대화를 봐야 하는 항목과 기한이 지난 항목은 모아서 한 번에 묻는다. 볼 장면이 없던 항목은 기한을 늘려 사용자에게 장면을 요청한다. `/slack-feedback`은 슬랙에 달린 피드백을 원인별로 묶어 대기열과 확인 목록에 합친다. 두 스킬에서 나온 새 일은 다시 대기열로 간다.
...
## 8. 공개 README 갱신을 묻는 훅 (2026-09-23)

random-ai-companion은 코드 repo를 비공개로 두고, README와 이미지만 담은 공개 repo를 따로 둔다([design-build-skills.md 공개 범위 기준](design-build-skills.md#공개-범위-기준)). README 원본이 비공개 repo에 있어서 README를 고친 PR을 머지하면 복사 스크립트를 돌려 공개 repo에 따로 커밋해야 한다. 이 절차가 CLAUDE.md 문장으로만 있으면 머지를 마친 세션이 마무리 절차를 따라가다 놓칠 수 있고, 그러면 공개 repo에는 이전 README가 그대로 있게 된다. README를 고칠 때마다 물으면 PR 여러 개에 걸쳐 다듬는 동안 매번 답해야 해서, 리뷰를 거쳐 main에 들어간 README를 최종본으로 생각하고 묻는 시점을 README 변경이 main에 머지된 뒤로 정했다.

§3 기준대로 이 문장을 훅으로 내렸고, 이 훅은 명령을 막는 대신 세션에 알림을 넣는다. 프로젝트 `.claude/settings.json`에 두 개를 걸었다.

| 훅 | 도는 때 | 알림을 받은 모델이 묻는 때 |
|----|--------|--------------------------|
| PostToolUse (Bash) | 명령이 `gh pr merge`일 때만, `-R`로 repo를 지정한 경우 포함 | 작업을 마무리해 보고할 때 |
| SessionStart | 세션을 새로 열거나 이어서 열 때마다 | 첫 답변에서 |

두 훅은 같은 스크립트를 부르고, 스크립트는 main README의 공개 본문과 본문이 쓰는 이미지를 공개 repo의 것과 비교해 다를 때만 알림을 넣는다. 머지라는 이벤트를 알아채는 대신 두 repo의 지금 상태를 비교하기 때문에, 웹 화면에서 머지했거나 머지 직후 알림을 놓쳤어도 다음 세션을 열 때 다시 잡힌다. PR diff에 README가 있는지로 판단하는 방식은 머지 순간을 놓치면 되찾을 방법이 없어서 쓰지 않았다. 사용자가 나중에 하겠다고 하면 스크립트가 지금 두 repo의 README와 이미지를 묶은 지문을 git 디렉터리에 적고, 어느 한 쪽이 바뀔 때까지 두 훅은 알리지 않는다. main 쪽 지문만 적으면 README를 되돌리는 PR이 머지됐을 때 공개 repo와 달라도 보류된 채로 남아서 두 쪽을 함께 적는다.

이 규칙은 그 프로젝트에만 해당해서 전역 settings나 플러그인 대신 프로젝트 `.claude/settings.json`에 두었다. 전역에 걸면 모든 프로젝트의 Bash 호출마다 그 repo인지 확인해야 하고, 프로젝트에 두면 repo를 받는 곳마다 훅도 같이 따라온다. 컨텍스트 비용은 알림이 나올 때 들어가는 문단 하나뿐이고, 세션을 열 때는 git fetch와 GitHub API 호출 두 번 때문에 1초 안팎이 더 걸린다. `claude -p`로 새 세션을 열어 두 훅의 알림이 모델의 컨텍스트에 들어가는 것까지 확인했다.

차단 훅은 명령이 실행되지 않는 것까지 보장하지만, 이 훅이 보장하는 것은 알림이 나오는 시점까지이고 묻는 일은 모델이 한다. 모델이 알림을 받고도 묻지 않으면 보류가 적히지 않으니 다음 세션을 열 때 다시 알린다. 훅 설정은 세션을 시작할 때 읽어서 그 repo 밖의 폴더에서 연 세션에서는 훅이 돌지 않고, 그 세션이 머지한 README는 그 repo에서 다음 세션을 열 때 알린다. `--auto`로 걸어 둔 머지가 나중에 끝나거나 서브에이전트가 머지한 경우도 같은 방식으로 다음 세션에서 잡힌다.

사용자에게 물을 일을 훅으로 내릴 때는 훅이 알림이 나오는 시점만 정하고 답은 사용자에게 받는다. 그 시점을 이벤트로 잡으면 이벤트를 놓쳤을 때 되찾을 수 없으니, 지금 상태를 비교해서 놓친 경우가 다음 비교에서 다시 잡히게 하는 쪽이 낫다.
