...
graph LR
    ...
    RD --> PS["지난 날짜 줄 정리<br/>끝난 지금 · 남은 작업 항목 지우기"]
    PS --> DU["사용자가 정해야 하는 일<br/>다시 볼 날짜가 된 남은 작업"]
...
graph LR
    ...
    TR -->|볼 장면 0건| SCENE["기한 7일 늘리기<br/>실행마다 장면 요청"]
    ...
    class SL io
    class PRE,OK,TR,AUTO,ASK,SCENE,FB judge
...
- */track 판정*: 항목마다 적힌 확인 방법과 닫는 기준으로 판정한다. 조회로 판정할 수 있는 항목은 직접 닫고, 사람이 봐야 하는 항목 가운데 닫거나 작업으로 옮길 것만 번호 목록으로 보여 승인을 받는다. 판단이 갈리는 항목만 질문으로 묻고, 기한까지 그대로 둘 항목은 수만 알린다. 사용자는 다르게 할 번호만 답한다. 기준이 보려는 상황이 한 번도 없던 항목은 닫지 않고 기한을 7일 늘린 뒤, 실행할 때마다 사용자에게 그 장면을 만들어 달라고 요청한다. DB 조회는 스킬 설정이 허용해 두어서 조회마다 묻지 않는다.
