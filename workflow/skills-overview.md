...
| `/feedback` | 슬랙에 달린 피드백을 원인별로 묶어 대기열과 확인 목록에 합친다 |
...
새 프로젝트는 `/init-project`로 기반을 만든다. 기능 단위 작업은 `/design`이 대기열에 세션 행을 넣고, 새 세션에서 `/next`나 `/build <세션>`으로 행을 하나씩 열어 구현한다. 머지한 세션은 배포 대기 표에 들어가고, 같은 배포 묶음이 다 모이면 `/deploy`로 운영에 올린다. `/track`이 배포 뒤 확인 목록을 판정하고, 여기서 나온 새 일과 `/feedback`이 묶은 피드백은 다시 대기열로 간다.
...
## /deploy · /track · /feedback

머지 뒤의 일은 스킬 3개가 나눠 맡는다. `/deploy`는 배포 대기 표를 비우고, `/track`은 배포 뒤 확인 목록을 판정하고, `/feedback`은 운영에서 들어온 피드백을 대기열로 가져온다.
...
graph LR
    ...
    TR -->|대화 · 기한 지남 · 기준 없음| ASK["모아서 한 번에 질문"]
    ASK -.작업으로 옮기기.-> QU[("LOCAL-SESSIONS.md<br/>남은 작업 · 대기열")]
    SL(["슬랙 피드백"]) -->|/feedback| FB["원인별로 묶기<br/>워터마크 뒤 새 행만"]
    FB --> QU
...
- */track 판정*: 항목마다 적힌 확인 방법과 닫는 기준으로 판정한다. 조회로 판정할 수 있는 항목은 직접 닫고, 사람이 봐야 하는 항목만 번호 목록으로 모아서 묻는다. 사용자는 제안과 다르게 할 번호만 답한다.
- */feedback 워터마크*: 마지막으로 반영한 피드백 행 번호를 대기열 파일에 남겨서, 다음에는 그 뒤에 들어온 행만 가져온다. 원인이 같은 행은 대기 중인 세션이나 남은 작업에 합치고, 합칠 곳이 없을 때만 새 세션을 만든다.
