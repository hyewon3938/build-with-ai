...
- 대기열에서 행을 지우고 남은 행의 순서 번호를 다시 매긴다. 배포 대기 표에는 세션, 이슈, PR, 머지한 날, 배포 묶음, 배포 때 할 일을 채운 행을 넣는다. 배포 때 할 일은 행의 배포 묶음 칸과 PR 본문에서 가져오고, 없으면 없음으로 적는다. 배포 묶음 칸이 비어 있어 배포할 것이 없는 세션이면 배포 대기 표에 넣지 않는다.
